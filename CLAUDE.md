...
| Infraestructura | Railway (backend + BD) + Vercel (admin y tablet). Nueva y separada de `app-conta`. Guía paso a paso en [docs/DEPLOY.md](docs/DEPLOY.md) |
...
Tras cambiar `prisma/schema.prisma` o hacer pull con migraciones nuevas: `npx prisma generate` (o `npm install`, que lo ejecuta vía `postinstall` del backend) y `npm run db:migrate`.
...
- `PORT` — puerto de la API del backend (en Railway la inyecta la plataforma).
...
- `CORS_ORIGINS` — solo producción: lista blanca de orígenes separados por comas (los dominios de Vercel del panel y la tablet). Sin definir, CORS queda abierto (desarrollo).

En los frontends de Vercel (admin y tablet), `VITE_API_URL` apunta al backend de Railway y se incrusta en el build (ver [docs/DEPLOY.md](docs/DEPLOY.md)).
...
- **Hecho (preparación de despliegue, el 2026-07-10):** backend listo para producción (CORS con lista blanca por `CORS_ORIGINS`, healthcheck `/api/health` con ping a BD, log de una línea por petición, `express.json` a 1 MB, `trust proxy`, `postinstall` con `prisma generate` y script `db:deploy`), `railway.json` (install/build sin las dependencias de Electron, migraciones automáticas en pre-deploy, healthcheck) y `vercel.json` en `admin/` y `tablet/` (install/build desde la raíz del monorepo, rewrite SPA). La tablet toma su `serverUrl` por defecto de `VITE_API_URL`. Guía de despliegue clic a clic (Railway + Vercel + instaladores/firma del desktop) en [docs/DEPLOY.md](docs/DEPLOY.md).
